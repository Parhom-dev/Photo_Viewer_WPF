    ...
    partial class Model
    {
        ...
        public void OpenImageFromPath(string fileName)
        {
            ...
            if(fileName.IndexOf("http") > -1)
            {
                ...
                {
                    client.DownloadFile(new Uri(fileName), @"download.jpg");
            ...
            try
            {
                var currentDir = new DirectoryInfo(Path.GetDirectoryName(download_url));
                if (fileName.IndexOf("http") > -1) download_url = currentDir.FullName + "\\download.jpg";
                _imagePaths = currentDir.EnumerateFiles()
                    ...
                    .ToList();
                var currentImage = _imagePaths.Single(path => path.Equals(download_url, StringComparison.OrdinalIgnoreCase));
                CurrentIndex = _imagePaths.IndexOf(currentImage);
                UpdateImage();
            } catch(Exception ex)
            {
                MessageBox.Show("Input correct image address!");
            }
        }
